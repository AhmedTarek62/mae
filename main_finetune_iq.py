...
import models_vit_iq
from advanced_finetuning.lora import create_lora_model  # keep LoRA only

from engine_finetune_iq import train_one_epoch, evaluate
from dataset_classes import IQDatasetH5, RML, RFPrintDataset
from dataset_classes.rml import make_snr_sampler


def get_args_parser():
    parser = argparse.ArgumentParser('IQ MAE fine-tuning for AoA/AMC', add_help=False)
    parser.add_argument('--batch_size', default=64, type=int)
    parser.add_argument('--epochs', default=50, type=int)
    parser.add_argument('--accum_iter', default=1, type=int)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--mode', type=str, default='amc', choices=['aoa', 'amc', 'rml', 'rfp'])

    # Model parameters
    parser.add_argument('--model', default='vit_iq_micro', type=str, metavar='MODEL')
    ...
    # Augmentation / Mixup (add missing args with safe defaults)
...
def main(args):
    ...
    # dataset (no time_mask needed; engine handles None)
    assert args.mode in ('aoa', 'amc', 'rml', 'rfp'), "mode must be amc, rml, aoa or rfp"
    if args.mode == 'amc' or args.mode == 'aoa':
        _, dataset = random_split(IQDatasetH5(args.data_path, mode=args.mode), [0.5, 0.5])
        ...
        raise ValueError(f"Unrecognized mode: {args.mode} - Supported modes are: amc, aoa, rml, rfp")
    ...
    # model (drop_path_rate not used by IQ ViT; omit to avoid mismatch)
    if args.mode == 'amc':
        nb_classes = 7
    elif args.mode == 'aoa':
        nb_classes = 3
    ...
    model = models_vit_iq.__dict__[args.model](
        global_pool=args.global_pool, num_classes=nb_classes, tanh=args.tanh
    )
    ...
    # unfreeze tokenizer for IQ
    # if hasattr(model, 'unfreeze_segment_embed'):
    #     model.unfreeze_segment_embed()
    ...
    # criterion
    if args.smoothing > 0. and args.mode == 'amc':
        criterion = LabelSmoothingCrossEntropy(smoothing=args.smoothing)
    else:
        criterion = torch.nn.MSELoss() if args.mode == 'aoa' else torch.nn.CrossEntropyLoss()
